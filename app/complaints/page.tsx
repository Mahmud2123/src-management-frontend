...
import {
  ...
  SlidersHorizontal, RefreshCw, Download, TrendingUp
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-green-50/30 p-6">
      <div className="max-w-7xl mx-auto space-y-6">
          ...
          <div className="space-y-4">
            ...
            {showFilters && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 pt-4 border-t border-gray-200">
                ...
                {(user?.role === 'ADMIN' || user?.role === 'SRC_EXECUTIVE') && (
                  <Select
                    label="View Scope"
                    value={roleFilter}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Total', value: complaints.length, color: 'bg-blue-100 text-blue-700', icon: FileText },
            { label: 'Pending', value: complaints.filter(c => c.status === 'PENDING').length, color: 'bg-yellow-100 text-yellow-700', icon: Clock },
            { label: 'In Progress', value: complaints.filter(c => c.status === 'IN_PROGRESS').length, color: 'bg-orange-100 text-orange-700', icon: TrendingUp },
            { label: 'Resolved', value: complaints.filter(c => c.status === 'RESOLVED').length, color: 'bg-green-100 text-green-700', icon: CheckCircle },
          ].map((stat, idx) => (
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-4 border-t border-gray-100 text-sm text-gray-600">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="truncate">{complaint.isAnonymous ? 'Anonymous' : complaint.author?.name}</span>
                    </div>
                    <div className="flex items-center gap-2">
