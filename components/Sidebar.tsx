...
export default function Sidebar() {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-white border-r border-gray-200">
        ...
        {visibleNav.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={`flex-1 font-medium ${active ? 'text-white' : 'text-gray-700'}`}>
                {item.name}
              </span>

              {badgeValue > 0 && (
                <span 
                  className={`flex items-center justify-center min-w-[20px] h-5 px-1.5 text-[10px] font-bold rounded-full border-2 ${
                    active 
      ...
      {/* Footer Actions - No Dark Mode Toggle */}
      <div className="p-4 border-t border-gray-200 space-y-2">
        <button
          onClick={() => handleNavigation('/settings')}
          className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-gray-700 hover:bg-gray-50 transition-all duration-200"
        >
