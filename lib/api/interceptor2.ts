...
apiClient.interceptors.response.use(
  ...
  (error: AxiosError) => {
    ...
    if (status === 403) {
      const isMaintenance = errorData?.maintenance || (typeof errorData?.message === 'string' && errorData.message.toLowerCase().includes('maintenance'));
      if (isMaintenance) {
        if (typeof window !== 'undefined') {
          const currentPath = window.location.pathname;
    ...
    if (!error.response) {
      ...
      if (error.code === 'ECONNABORTED') customMessage = 'Request timed out. Please try again.';
      else if (error.code === 'ENOTFOUND' || error.code === 'ECONNREFUSED') customMessage = 'Unable to connect to the server. Please try again later.';
